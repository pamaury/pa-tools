...
typedef void (*usb_xfer_completion_fn_t)(bool in, unsigned len, void *user);

usb_xfer_completion_fn_t endp_completion[2];
void *endp_completion_user[2];

static void usb_xfer(void *ptr, unsigned len, int send, usb_xfer_completion_fn_t fn, void *user)
{
    ...
    endp_completion[send] = fn;
    endp_completion_user[send] = user;

    REG_ENDPTPRIME |= mask;
}

static void usb_send(void *buffer, unsigned len, usb_xfer_completion_fn_t fn, void *user)
{
    usb_xfer(buffer, len, true, fn, user);
}

static void usb_recv(void *buffer, unsigned len, usb_xfer_completion_fn_t fn, void *user)
{
    usb_xfer(buffer, len, false, fn, user);
}
...
static void handle_get_desc(struct usb_ctrlrequest *req)
{
    ...
    usb_recv(NULL, 0, NULL, NULL);
    usb_send(response_data, length, NULL, NULL);
}

static void set_address_cb(bool send, unsigned len, void *user)
{
    REG_DEVICEADDR = (unsigned int)user;
}
...
static void handle_std_setup(struct usb_ctrlrequest *req)
{
    switch(req->bRequest)
    {
        case USB_REQ_SET_ADDRESS:
            usb_send(NULL, 0, &set_address_cb, (void *)(req->wValue << USBDEVICEADDRESS_BIT_POS));
            break;
        case USB_REQ_GET_DESCRIPTOR:
            handle_get_desc(req);
            break;
        case USB_REQ_SET_CONFIGURATION:
            usb_send(NULL, 0, NULL, NULL);
            break;
        default:
            usb_stall();
            break;
...
static void handle_vendor_setup(struct usb_ctrlrequest *req)
{
            ...
            (req->bRequestType & USB_RECIP_MASK) == USB_RECIP_DEVICE)
    {
        if(req->bRequest == 0x99)
        {
            ...
            usb_recv(NULL, 0, NULL, NULL);
            usb_send(response_data, len, NULL, NULL);
        }
        else
            usb_stall();
    }
...
static void do_transfer_completed(int in)
{
    if(endp_completion[in])
        endp_completion[in](in, 0, endp_completion_user[in]);
    endp_completion[in] = NULL;
}
...
static void transfer_completed(void)
{
    ...
    if(mask & 1)
        do_transfer_completed(false);
    if(mask & 0x10000)
        do_transfer_completed(true);
