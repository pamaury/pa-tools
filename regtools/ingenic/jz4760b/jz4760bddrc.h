...
#define DDRC_DQSADJ	(DDRC_BASE + 0x20) /* DDR DQS Delay Adjust Register */
/* n (0-1) */
#define DDRC_MMAP(n)	(DDRC_BASE + 0x24 + 4*(n)) /* DDR Memory Map Config Register */
...
#define REG_DDRC_DQSADJ	REG32(DDRC_DQSADJ)
#define REG_DDRC_MMAP(n)		REG32(DDRC_MMAP(n))
// #define REG_DDRC_MMAP0		REG32(DDRC_MMAP0)
// #define REG_DDRC_MMAP1		REG32(DDRC_MMAP1)

/* DDRC Status Register */
#define DDRC_ST_ENDIAN	(1 << 7) /* 0 Little data endian, 1 Big data endian */
#define DDRC_ST_DPDN		(1 << 5) /* 0 DDR memory is NOT in deep-power-down state, 1 DDR memory is in deep-power-down state */
#define DDRC_ST_PDN		(1 << 4) /* 0 DDR memory is NOT in power-down state, 1 DDR memory is in power-down state */
#define DDRC_ST_AREF		(1 << 3) /* 0 DDR memory is NOT in auto-refresh state, 1 DDR memory is in auto-refresh state */
#define DDRC_ST_SREF		(1 << 2) /* 0 DDR memory is NOT in self-refresh state, 1 DDR memory is in self-refresh state */
#define DDRC_ST_CKE1		(1 << 1) /* 0 CKE1 Pin is low, 1 CKE1 Pin is high */
#define DDRC_ST_CKE0		(1 << 0) /* 0 CKE0 Pin is low, 1 CKE0 Pin is high */
...
#define DDRC_CFG_ROW1_MASK	(3 << DDRC_CFG_ROW1_BIT)
#define DDRC_CFG_COL1_BIT	25 /* Row Address width. */
#define DDRC_CFG_COL1_MASK	(3 << DDRC_CFG_COL1_BIT)
...
#define DDRC_CFG_COL_BIT	8 /* Column Address width. Specify the Column address width of external DDR. */
...
#define DDRC_CFG_CS1EN	(1 << 7) /* 0 DDR Pin CS1 un-used, 1 There're DDR memory connected to CS1 */
#define DDRC_CFG_CS0EN	(1 << 6) /* 0 DDR Pin CS0 un-used, 1 There're DDR memory connected to CS0 */
...
#define DDRC_CFG_BA		(1 << 1) /* 0 4 bank device, Pin ba[1:0] valid, ba[2] un-used, 1 8 bank device, Pin ba[2:0] valid*/
#define DDRC_CFG_DW		(1 << 0) /*0 External memory data width is 16-bit, 1 External memory data width is 32-bit */

/* DDRC Control Register */
#define DDRC_CTRL_ACTPD	(1 << 15) /* 0 Precharge all banks before entering power-down, 1 Do not precharge banks before entering power-down */
...
#define DDRC_CTRL_SR		(1 << 5) /* 1 Drive external DDR device entering self-refresh mode, 0 Drive external DDR device exiting self-refresh mode */
#define DDRC_CTRL_UNALIGN	(1 << 4) /* 0 Disable unaligned transfer on AXI BUS, 1 Enable unaligned transfer on AXI BUS */
#define DDRC_CTRL_ALH		(1 << 3) /* Advanced Latency Hiding: 0 Disable ALH, 1 Enable ALH */
#define DDRC_CTRL_RDC		(1 << 2) /* 0 dclk clock frequency is lower than 60MHz, 1 dclk clock frequency is higher than 60MHz */
...
#define DDRC_LMR_DDR_ADDR_BIT	16 /* When performing a DDR command, DDRC_ADDR[13:0] corresponding to external DDR address Pin A[13:0] */
#define DDRC_LMR_DDR_ADDR_MASK	(0xff << DDRC_LMR_DDR_ADDR_BIT)

#define DDRC_LMR_BA_BIT		8 /* When performing a DDR command, BA[2:0] corresponding to external DDR address Pin BA[2:0]. */
#define DDRC_LMR_BA_MASK	(0x7 << DDRC_LMR_BA_BIT)
  /* For DDR2 */
  #define DDRC_LMR_BA_MRS	(0 << DDRC_LMR_BA_BIT) /* (For DDR2) Mode Register set */
  #define DDRC_LMR_BA_EMRS1	(1 << DDRC_LMR_BA_BIT) /* (For DDR2) Extended Mode Register1 set */
  #define DDRC_LMR_BA_EMRS2	(2 << DDRC_LMR_BA_BIT) /* (For DDR2) Extended Mode Register2 set */
  #define DDRC_LMR_BA_EMRS3	(3 << DDRC_LMR_BA_BIT) /* (For DDR2) Extended Mode Register3 set */
  /* For mobile DDR */
  #define DDRC_LMR_BA_M_MRS	(0 << DDRC_LMR_BA_BIT) /* (For mobile DDR) Mode Register set */
  #define DDRC_LMR_BA_M_EMRS	(2 << DDRC_LMR_BA_BIT) /* (For mobile DDR) Extended Mode Register set */
  #define DDRC_LMR_BA_M_SR	(1 << DDRC_LMR_BA_BIT) /* (For mobile DDR) Status Register set */
...
#define DDRC_LMR_START		(1 << 0) /* 0 No command is performed, 1 On the posedge of START, perform a command defined by CMD field */

/* DDRC Mode Register Set */
#define DDR_MRS_PD_BIT		10 /* Active power down exit time */
#define DDR_MRS_PD_MASK		(1 << DDR_MRS_PD_BIT) 
  #define DDR_MRS_PD_FAST_EXIT	(0 << DDR_MRS_PD_BIT)
  #define DDR_MRS_PD_SLOW_EXIT	(1 << DDR_MRS_PD_BIT)
#define DDR_MRS_WR_BIT		9 /* Write Recovery for autoprecharge */
#define DDR_MRS_WR_MASK		(7 << DDR_MRS_WR_BIT)
#define DDR_MRS_DLL_RST		(1 << 8) /* DLL Reset */
#define DDR_MRS_TM_BIT		7        /* Operating Mode */
#define DDR_MRS_TM_MASK		(1 << DDR_MRS_TM_BIT) 
  #define DDR_MRS_TM_NORMAL	(0 << DDR_MRS_TM_BIT)
  #define DDR_MRS_TM_TEST	(1 << DDR_MRS_TM_BIT)
...
#define DDR_EMRS1_QOFF		(1<<12) /* 0 Output buffer enabled, 1 Output buffer disabled */
...
#define DDR_EMRS1_RTT_MASK	(0x11 << DDR_EMRS1_RTT_BIT) /* Bit 6, Bit 2 */
...
#define DDRC_DQSADJWDQS_BIT          8 /* The adjust value for WRITE DQS delay */
#define DDRC_DQSADJWDQS_MASK         (0x1f << DDRC_DQSADJWDQS_BIT)
#define DDRC_DQSADJRDQS_BIT          0 /* The adjust value for READ DQS delay */
...
#define DDRC_MMAP_BASE_64M	(0x24 << DDRC_MMAP_BASE_BIT) /*when bank0 is 128M*/
#define DDRC_MMAP_BASE_128M	(0x28 << DDRC_MMAP_BASE_BIT) /*when bank0 is 128M*/
#define DDRC_MMAP_BASE_256M	(0x30 << DDRC_MMAP_BASE_BIT) /*when bank0 is 128M*/
